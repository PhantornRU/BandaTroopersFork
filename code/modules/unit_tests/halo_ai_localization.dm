/proc/halo_unit_test_assert_localized_lines(datum/unit_test/test, list/lines, context, allow_empty = FALSE)
	var/static/list/allowed_emotes = list("*warcry", "*pain", "*scream")
	var/static/regex/latin_regex = regex("\[A-Za-z\]")

	if(!allow_empty)
		if(!length(lines))
			test.Fail("[context] unexpectedly resolved to an empty line list.", __FILE__, __LINE__)
			return

	for(var/line as anything in lines)
		if(!istext(line))
			test.Fail("[context] contains a non-text entry: [line]", __FILE__, __LINE__)
			return
		...
		if(latin_regex.Find(line))
			test.Fail("[context] leaked a Latin-script line: [line]", __FILE__, __LINE__)
			return
...
/datum/unit_test/halo_ai_localization_faction_banks/Run()
	...
	for(var/faction_name as anything in representative_factions)
		...
		var/list/expected_pack = halo_ai_get_faction_localization_pack(faction_name)
		...
		for(var/key in halo_ai_line_pack_keys())
			...
			halo_unit_test_assert_localized_lines(src, actual_lines, "[faction_name] [key]")
...
/datum/unit_test/halo_ai_localization_brain_fallback/Run()
	...
	halo_unit_test_assert_localized_lines(src, deathsquad_brain.enter_combat_lines, "WY deathsquad enter_combat_lines")
	halo_unit_test_assert_localized_lines(src, deathsquad_brain.reload_lines, "WY deathsquad reload fallback")
	...
	for(var/key in halo_ai_line_pack_keys())
		...
		halo_unit_test_assert_localized_lines(src, brain_lines, "Malf synth fallback [key]")
