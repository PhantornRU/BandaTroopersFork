...
/datum/unit_test/halo_equip_test/proc/prepare_test_human_for_squad(mob/living/carbon/human/human, preset_type = /datum/equipment_preset, preset_assignment = null)
	...
	id.assignment = preset.assignment
	id.rank = preset.rank
	id.registered_name = human.real_name
	id.registered_ref = WEAKREF(human)
	id.registered_gid = human.gid
	id.blood_type = human.blood_type
	id.name = "[human.real_name]'s [id.card_name]"
	if(id.assignment)
...
/datum/unit_test/halo_integration_test/proc/find_vendor_entry_by_product_type(list/listed_products, product_type)
	if(!islist(listed_products) || !product_type)
		return null

	for(var/list/entry as anything in listed_products)
		if(length(entry) >= 3 && entry[3] == product_type)
			return entry

	return null

/datum/unit_test/halo_integration_test/proc/find_orbit_entry_by_name(list/entries, target_name)
	if(!islist(entries) || !target_name)
		return null

	for(var/list/entry as anything in entries)
		if(entry["full_name"] == target_name)
			return entry
...
/datum/unit_test/halo_ship_platoons/odst_leader_assignment_labels/Run()
	...
	TEST_ASSERT_EQUAL(manager.rename_squad(alpha_squad, "War Cogs Vanguard", null, "unit_test_odst_assignment", TRUE), TRUE, "ODST assignment label rename failed during runtime label testing.")
...
/datum/unit_test/halo_ship_platoons/odst_preview_dummy_slots/Run()
	...
	prefs.job_preference_list = list(JOB_SQUAD_MARINE_ODST = HIGH_PRIORITY)
...
/datum/unit_test/halo_ship_platoons_halo_prep_vendors_include_m7_attachments
	parent_type = /datum/unit_test/halo_integration_test

/datum/unit_test/halo_ship_platoons_halo_prep_vendors_include_m7_attachments/Run()
	var/obj/structure/machinery/cm_vending/sorted/cargo_guns/squad/unsc/unsc_vendor = track_test_atom(allocate(/obj/structure/machinery/cm_vending/sorted/cargo_guns/squad/unsc, run_loc_floor_top_right))
	var/obj/structure/machinery/cm_vending/sorted/cargo_guns/squad/unsc/odst/odst_vendor = track_test_atom(allocate(/obj/structure/machinery/cm_vending/sorted/cargo_guns/squad/unsc/odst, run_loc_floor_bottom_left))

	unsc_vendor.populate_product_list(1)
	odst_vendor.populate_product_list(1)

	var/list/unsc_flashlight_entry = find_vendor_entry_by_product_type(unsc_vendor.listed_products, /obj/item/attachable/flashlight/m7)
	var/list/odst_flashlight_entry = find_vendor_entry_by_product_type(odst_vendor.listed_products, /obj/item/attachable/flashlight/m7)
	var/list/odst_reddot_entry = find_vendor_entry_by_product_type(odst_vendor.listed_products, /obj/item/attachable/reddot/m7)

	TEST_ASSERT_NOTNULL(unsc_flashlight_entry, "The HALO UNSC utility vendor no longer lists the M7 flashlight attachment.")
	TEST_ASSERT_EQUAL(unsc_flashlight_entry[2], 1, "The HALO UNSC utility vendor should stock exactly one M7 flashlight per list build.")
	TEST_ASSERT_NOTNULL(odst_flashlight_entry, "The HALO ODST utility vendor no longer lists the M7 flashlight attachment.")
	TEST_ASSERT_EQUAL(odst_flashlight_entry[2], 1, "The HALO ODST utility vendor should stock exactly one M7 flashlight per list build.")
	TEST_ASSERT_NOTNULL(odst_reddot_entry, "The HALO ODST utility vendor no longer lists the M7 red dot attachment.")
	TEST_ASSERT_EQUAL(odst_reddot_entry[2], 1, "The HALO ODST utility vendor should stock exactly one M7 red dot per list build.")

/datum/unit_test/halo_ship_platoons_zombie_presets_keep_claws
	parent_type = /datum/unit_test/halo_integration_test

/datum/unit_test/halo_ship_platoons_zombie_presets_keep_claws/Run()
	var/mob/living/carbon/human/zombie = create_test_human("Zombie Claw Regression", JOB_COLONIST)
	arm_equipment(zombie, /datum/equipment_preset/zombie/generic, TRUE, TRUE)

	TEST_ASSERT(istype(zombie.get_item_by_slot(WEAR_L_HAND), /obj/item/weapon/zombie_claws), "Zombie preset spawning no longer equips claws in the left hand.")
	TEST_ASSERT(istype(zombie.get_item_by_slot(WEAR_R_HAND), /obj/item/weapon/zombie_claws), "Zombie preset spawning no longer equips claws in the right hand.")

/datum/unit_test/halo_ship_platoons_follow_groups_unsc_squads
	parent_type = /datum/unit_test/halo_integration_test

/datum/unit_test/halo_ship_platoons_follow_groups_unsc_squads/Run()
	configure_test_ship_platoon(/datum/squad/marine/halo/unsc/alpha)

	var/mob/living/carbon/human/unsc_human = create_test_human("HALO Orbit UNSC", JOB_SQUAD_MARINE_UNSC, /datum/squad/marine/halo/unsc/alpha, run_loc_floor_top_right, "halo_orbit_unsc")
	arm_equipment(unsc_human, /datum/equipment_preset/unsc/pfc, FALSE, TRUE)
	var/mob/living/carbon/human/odst_human = create_test_human("HALO Orbit ODST", JOB_SQUAD_MARINE_ODST, /datum/squad/marine/halo/odst/alpha, run_loc_floor_top_right, "halo_orbit_odst")
	arm_equipment(odst_human, /datum/equipment_preset/unsc/pfc/odst, FALSE, TRUE)

	var/mob/dead/observer/observer = track_test_atom(allocate(/mob/dead/observer, run_loc_floor_bottom_left))
	var/datum/orbit_menu/menu = new(observer)
	var/list/data = menu.ui_static_data(observer)
	var/list/unsc_entry = find_orbit_entry_by_name(data["marines"], unsc_human.real_name)
	var/list/odst_entry = find_orbit_entry_by_name(data["marines"], odst_human.real_name)

	TEST_ASSERT_NOTNULL(unsc_entry, "HALO UNSC squad members no longer appear in the grouped Marines section of Follow/Orbit.")
	TEST_ASSERT_EQUAL(unsc_entry["squad_runtime"], unsc_human.assigned_squad?.name, "HALO UNSC Follow/Orbit entries lost their runtime squad name.")
	TEST_ASSERT_EQUAL(unsc_entry["squad_static"], GLOB.squad_name_manager?.get_static_name_by_squad(unsc_human.assigned_squad), "HALO UNSC Follow/Orbit entries lost their static squad mapping.")
	TEST_ASSERT_NULL(find_orbit_entry_by_name(data["humans"], unsc_human.real_name), "HALO UNSC squad members incorrectly fell back into the generic Humans section of Follow/Orbit.")
	TEST_ASSERT_NOTNULL(odst_entry, "HALO ODST squad members no longer appear in the grouped Marines section of Follow/Orbit.")
	TEST_ASSERT_EQUAL(odst_entry["squad_runtime"], odst_human.assigned_squad?.name, "HALO ODST Follow/Orbit entries lost their runtime squad name.")
	TEST_ASSERT_EQUAL(odst_entry["squad_static"], GLOB.squad_name_manager?.get_static_name_by_squad(odst_human.assigned_squad), "HALO ODST Follow/Orbit entries lost their static squad mapping.")
...
/datum/unit_test/halo_ship_platoons/halo_squad_armory_button_access/Run()
	...
	var/mob/living/carbon/human/unsc_captain = create_test_human("HALO UNSC Captain", "UNSC Captain")
	arm_equipment(unsc_captain, /datum/equipment_preset/unsc_crew/command/xo/cpt, FALSE, TRUE)
	var/mob/living/carbon/human/crew_human = create_test_human("HALO Armory Crew", "UNSC Crewman")
	arm_equipment(crew_human, /datum/equipment_preset/unsc_crew/generic, FALSE, TRUE)
	...
	TEST_ASSERT(button.allowed(unsc_captain), "The HALO UNSC captain should be able to open an unavailable squad armory button.")
	TEST_ASSERT(button.allowed(crew_human), "HALO crew roles should be able to open an unavailable squad armory button once their ship access is configured.")
...
/datum/unit_test/halo_ship_platoons_halo_crew_headsets_match_pltco_channels
	parent_type = /datum/unit_test/halo_integration_test

/datum/unit_test/halo_ship_platoons_halo_crew_headsets_match_pltco_channels/Run()
	var/mob/living/carbon/human/crew_human = create_test_human("HALO Crew Headset", "UNSC Crewman")
	arm_equipment(crew_human, /datum/equipment_preset/unsc_crew/generic, FALSE, TRUE)

	var/obj/item/device/radio/headset/almayer/marine/solardevils/pltco/unsc/crew_headset = crew_human.get_item_by_slot(WEAR_L_EAR)
	TEST_ASSERT(istype(crew_headset), "HALO crew presets should equip the platoon-command UNSC headset variant.")
	TEST_ASSERT_EQUAL(length(crew_headset.initial_keys), 2, "HALO crew headsets should inherit the same two encryption keys as the UNSC platoon commander headset.")

	var/obj/structure/closet/secure_closet/marine_personal/unsc_crew/crewman/crew_locker = track_test_atom(allocate(/obj/structure/closet/secure_closet/marine_personal/unsc_crew/crewman, run_loc_floor_top_right))
	TEST_ASSERT_EQUAL(count_personal_locker_contents_by_exact_type(crew_locker, /obj/item/device/radio/headset/almayer/marine/solardevils/pltco/unsc), 1, "The HALO crew locker should spawn the platoon-command UNSC headset.")
	TEST_ASSERT_EQUAL(count_personal_locker_contents_by_exact_type(crew_locker, /obj/item/device/radio/headset/almayer/marine/solardevils/unsc), 0, "The HALO crew locker should no longer spawn the baseline UNSC headset.")
