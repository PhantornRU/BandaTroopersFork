...
/datum/unit_test/halo_ship_platoons_cryo_squad_leader_final_state/Run()
	...
	var/expected_lead_icon = human.assigned_squad?.lead_icon || "leader"
	TEST_ASSERT(holder_has_overlay_state(holder, "hudsquad_[expected_lead_icon]"), "HALO squad-leader runtime cryo path did not rebuild the expected leader HUD indicator state.")
