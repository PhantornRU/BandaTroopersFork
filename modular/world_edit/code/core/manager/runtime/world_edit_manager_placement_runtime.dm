...
/datum/world_edit_manager/proc/resolve_placement_candidate(mob/user, turf/start_turf, turf/end_turf, list/runtime_params = null, hover_only = FALSE, list/shape_metadata_override = null, list/collector_state_summary = null, shape_id_override = null)
	...
	if(!current_generator)
		candidate.resolve_error = "Generator is not active."
		return candidate
	...
	if(!length(shape_contract.anchor_turfs))
		candidate.resolve_error = "Invalid placement footprint."
		return candidate
	...
	if(!istype(plan))
		candidate.resolve_error = "Failed to build placement plan."
		return candidate
	candidate.plan = plan
	...
	if(!length(plan.placements) && !length(plan.deletions))
		candidate.resolve_error = "Placement plan is empty."
		return candidate
...
/datum/world_edit_manager/proc/apply_resolved_placement_candidate(mob/user, datum/world_edit_placement_candidate/candidate = null)
...
/datum/world_edit_manager/proc/apply_safe_placement_current_plan(mob/user)
	return apply_resolved_placement_candidate(user, get_placement_preview_candidate())
