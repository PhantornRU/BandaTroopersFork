...
/datum/world_edit_generator/building_layout/proc/build_building_semantic_layout(datum/world_edit_building_layout_state/state)
	...
	build_building_requested_room_dividers(state)
...
/datum/world_edit_generator/building_layout/proc/assign_room_first_zone_rooms(datum/world_edit_building_layout_state/state, list/room_candidates, list/zone_specs, corridor_zone_id, entry_zone_id)
	...
	var/entry_assigned = FALSE
	var/hub_assigned = FALSE
	if(islist(root_selection))
		entry_assigned = root_selection["entry_assigned"]
		hub_assigned = root_selection["hub_assigned"]
	...
	for(var/datum/world_edit_building_zone_spec/zone_spec as anything in zone_specs)
		if(!istype(zone_spec))
			continue
		if(zone_spec.id == corridor_zone_id && hub_assigned)
			continue
		if(zone_spec.id == entry_zone_id && entry_assigned)
			continue
		...
		if(zone_spec.required)
			state.add_warning("Required zone '[zone_spec.id]' has no valid BSP room candidate. Remnant allocation will try before hub absorption.")
			continue
...
/datum/world_edit_generator/building_layout/proc/assign_room_first_entry_and_hub_rooms(datum/world_edit_building_layout_state/state, list/room_candidates, list/used_candidate_ids, corridor_zone_id, entry_zone_id)
	var/list/result = list("claimed_turfs" = list(), "entry_assigned" = FALSE, "hub_assigned" = FALSE)
	...
	if(islist(entry_candidate))
		...
		if(istype(entry_spec))
			...
			if(istype(entry_room))
				if(assigned_entry_zone_id == entry_zone_id)
					result["entry_assigned"] = TRUE
				if(assigned_entry_zone_id == corridor_zone_id)
					result["hub_assigned"] = TRUE
...
/datum/world_edit_generator/building_layout/proc/build_room_first_remnant_allowed_lookup(datum/world_edit_building_layout_state/state, list/free_lookup)
	var/list/allowed = list()
	if(!istype(state) || !islist(free_lookup))
		return allowed
	for(var/turf/free_turf as anything in get_room_first_sorted_free_turfs(free_lookup))
		if(!istype(free_turf))
			continue
		if(length(state.get_zone(free_turf)))
			continue
		if(!state.geometry.footprint_lookup[free_turf] || state.geometry.boundary_lookup[free_turf] || state.geometry.wall_lookup[free_turf] || state.geometry.door_dirs[free_turf])
			continue
		if(state.geometry.reserved_lookup[free_turf] || free_turf == state.geometry.front_door_turf)
			continue
		allowed[free_turf] = TRUE
	return allowed

/datum/world_edit_generator/building_layout/proc/build_room_first_remnant_seed_turfs(list/component, list/allowed_lookup, target_area, turf/preferred_turf = null)
	var/list/seed_turfs = list()
	if(!islist(component) || !islist(allowed_lookup) || target_area <= 0)
		return seed_turfs
	var/list/queue = list()
	if(istype(preferred_turf) && allowed_lookup[preferred_turf])
		queue += preferred_turf
	for(var/turf/component_turf as anything in component)
		if(istype(component_turf) && allowed_lookup[component_turf] && !(component_turf in queue))
			queue += component_turf
	var/list/visited = list()
	var/index = 1
	while(index <= length(queue) && length(seed_turfs) < target_area)
		var/turf/current = queue[index++]
		if(!istype(current) || visited[current] || !allowed_lookup[current])
			continue
		visited[current] = TRUE
		seed_turfs += current
		for(var/check_dir in GLOB.cardinals)
			var/turf/nearby = get_step(current, check_dir)
			if(allowed_lookup[nearby] && !visited[nearby] && !(nearby in queue))
				queue += nearby
	return seed_turfs

/datum/world_edit_generator/building_layout/proc/select_room_first_remnant_candidate(datum/world_edit_building_layout_state/state, list/free_lookup, datum/world_edit_building_zone_spec/zone_spec)
	if(!istype(state) || !islist(free_lookup) || !istype(zone_spec))
		return null
	var/target_area = max(zone_spec.min_area, 2)
	var/list/allowed_lookup = build_room_first_remnant_allowed_lookup(state, free_lookup)
	if(!length(allowed_lookup))
		return null
	var/list/allowed_turfs = get_room_first_sorted_free_turfs(allowed_lookup)
	var/list/components = extract_room_first_connected_components(allowed_lookup, allowed_turfs)
	var/list/best_candidate = null
	var/best_score = -999999999
	for(var/list/component as anything in components)
		if(!islist(component) || length(component) < target_area)
			continue
		for(var/turf/start_turf as anything in component)
			if(!istype(start_turf) || !allowed_lookup[start_turf])
				continue
			var/list/seed_turfs = build_room_first_remnant_seed_turfs(component, allowed_lookup, target_area, start_turf)
			if(length(seed_turfs) < target_area)
				continue
			var/list/candidate = build_room_first_candidate_from_turfs(state, seed_turfs, "room_remnant_candidate_[zone_spec.id]")
			if(!islist(candidate))
				continue
			var/route_access = 0
			for(var/turf/seed_turf as anything in seed_turfs)
				for(var/check_dir in GLOB.cardinals)
					var/turf/nearby = get_step(seed_turf, check_dir)
					if(state.geometry.corridor_lookup[nearby] || state.geometry.primary_route_turfs.Find(nearby) || length(state.get_zone(nearby)))
						route_access++
			candidate["route_access"] = route_access
			candidate["corridor_touch"] = max(round(text2num("[candidate["corridor_touch"]]") || 0), route_access)
			var/score = score_room_first_candidate_for_zone(state, candidate, zone_spec)
			if(route_access <= 0)
				score -= 25000
			if(score > best_score)
...
/datum/world_edit_generator/building_layout/proc/allocate_room_first_required_zone_remnants(datum/world_edit_building_layout_state/state, list/free_lookup, list/room_candidates, list/required_room_specs, corridor_zone_id, entry_zone_id)
	if(!istype(state) || !istype(state.semantic_plan) || !islist(free_lookup))
		return
	var/list/zone_specs = islist(required_room_specs) ? required_room_specs : state.semantic_plan.zone_specs
	for(var/datum/world_edit_building_zone_spec/zone_spec as anything in zone_specs)
		if(!istype(zone_spec) || !zone_spec.required)
			continue
		if(zone_spec.id == "[corridor_zone_id]" || zone_spec.id == "[entry_zone_id]")
			continue
		if(length(state.get_zone_turfs(zone_spec.id)) >= max(zone_spec.min_area, 1))
			continue
		var/list/candidate = select_room_first_remnant_candidate(state, free_lookup, zone_spec)
		if(!islist(candidate))
			state.add_warning("Required zone '[zone_spec.id]' still has no 4-connected remnant room candidate before hub absorption.")
			continue
		var/datum/world_edit_building_room/remnant_room = emit_room_first_room_for_candidate(state, candidate, zone_spec)
		if(!istype(remnant_room))
			continue
		candidate["used"] = TRUE
		if(islist(room_candidates))
			room_candidates += list(candidate)
		for(var/turf/claimed_turf as anything in candidate["turfs"])
			free_lookup -= claimed_turf
		state.add_warning("Required zone '[zone_spec.id]' allocated [length(candidate["turfs"])] 4-connected remnant turfs before hub absorption.")


/datum/world_edit_generator/building_layout/proc/allocate_room_first_required_zone_remnants_legacy_unused(datum/world_edit_building_layout_state/state)
	/// Fourth-pass allocation: when BSP candidate shortage leaves required zones
	/// with zero assigned turfs, this pass finds remnant footprint turfs from
	/// the hub/corridor zone and allocates them as minimal rooms so that anchor
	/// extraction and fixture placement have geometry to work with.
	if(!istype(state) || !istype(state.semantic_plan))
		return
	var/corridor_zone_id = select_room_first_corridor_zone_id(state)
	for(var/datum/world_edit_building_zone_spec/zone_spec as anything in state.semantic_plan.zone_specs)
		if(!istype(zone_spec) || !zone_spec.required)
			continue
		if(length(state.get_zone_turfs(zone_spec.id)) > 0)
			continue
		// Required zone has zero turfs — collect remnant candidates from hub territory
		var/turf_need = max(zone_spec.min_area, 3)
		var/list/remnant_turfs = list()
		var/list/used_lookup = list()
		var/try_limit = turf_need * 7
		for(var/attempt in 1 to try_limit)
			if(length(remnant_turfs) >= turf_need)
				break
			var/turf/best_turf = null
			var/best_score = -999999999
			for(var/turf/foot_turf as anything in state.geometry.floor_turfs)
				if(!istype(foot_turf) || !state.geometry.footprint_lookup[foot_turf])
					continue
				if(used_lookup[foot_turf])
					continue
				if(state.geometry.boundary_lookup[foot_turf] || state.geometry.door_dirs[foot_turf])
					continue
				var/current_zone = state.get_zone(foot_turf)
				if(current_zone == zone_spec.id)
					continue
				if(length(current_zone) && current_zone != corridor_zone_id)
					continue
				if(state.geometry.reserved_lookup[foot_turf] && length(current_zone))
					continue
				var/score = 0
				if(current_zone == corridor_zone_id)
					score += 5000
				if(istype(state.geometry.front_door_turf))
					score -= get_dist(foot_turf, state.geometry.front_door_turf) * 3
				if(istype(state.geometry.semantic_hub_turf))
					score -= get_dist(foot_turf, state.geometry.semantic_hub_turf) * 2
				if(state.geometry.wall_lookup[foot_turf])
					score -= 500
				if(score > best_score)
					best_turf = foot_turf
					best_score = score
			if(istype(best_turf))
				remnant_turfs += best_turf
				used_lookup[best_turf] = TRUE
			else
				break
		if(length(remnant_turfs))
			var/datum/world_edit_building_room/remnant_room = new("room_remnant_[zone_spec.id]", zone_spec.id, zone_spec.role)
			for(var/turf/rt as anything in remnant_turfs)
				remnant_room.add_turf(rt)
			remnant_room.focus_turf = state.geometry.semantic_hub_turf || state.geometry.front_door_turf || remnant_turfs[1]
			state.add_solved_room(remnant_room)
			state.add_warning("Required zone '[zone_spec.id]' had zero BSP candidates; allocated [length(remnant_turfs)] remnant turfs (fourth-pass remnant allocation).")
		else
			state.add_warning("Required zone '[zone_spec.id]' has zero turfs after fourth-pass remnant allocation — no remnant floor turfs available.")
...
/datum/world_edit_generator/building_layout/proc/rebuild_room_first_corridor_from_hub(datum/world_edit_building_layout_state/state, corridor_zone_id, entry_zone_id)
	...
	var/corridor_zone_is_hub = corridor_zone_id == "[state.semantic_plan?.hub_zone_id]" || corridor_zone_id == "[state.semantic_plan?.primary_zone_id]"
	if(corridor_zone_is_hub)
		var/list/route_floor_lookup = length(state.geometry.floor_lookup) ? state.geometry.floor_lookup : build_building_provisional_floor_lookup(state)
		var/turf/route_end = state.geometry.semantic_hub_turf || state.get_zone_focus(corridor_zone_id)
		if(istype(state.geometry.front_door_turf) && istype(route_end))
			for(var/turf/route_turf as anything in build_reserved_path(state.geometry.front_door_turf, route_end, route_floor_lookup))
				state.add_corridor_turf(route_turf)
		else if(istype(route_end))
			state.add_corridor_turf(route_end)
	else
...
/datum/world_edit_generator/building_layout/proc/ensure_room_first_emergency_door(datum/world_edit_building_layout_state/state, datum/world_edit_building_room/room)
	if(!istype(state) || !istype(room))
		return null
	var/turf/best_opening_turf = null
	var/turf/best_pair_turf = null
	var/best_dir = null
	var/best_target_zone_id = null
	var/best_score = -999999999
	var/turf/target_turf = state.geometry.semantic_hub_turf || state.geometry.front_door_turf || room.focus_turf
	for(var/turf/room_turf as anything in room.turfs)
		if(!istype(room_turf) || state.geometry.reserved_lookup[room_turf] || state.geometry.door_dirs[room_turf])
			continue
		for(var/check_dir in GLOB.cardinals)
			var/turf/nearby_turf = get_step(room_turf, check_dir)
			if(!istype(nearby_turf))
				continue
			// Accept any nearby turf that is inside the footprint, even if it's a wall or boundary
			if(!state.geometry.footprint_lookup[nearby_turf])
				continue
			var/nearby_zone_id = state.get_zone(nearby_turf)
			if(!length(nearby_zone_id) || nearby_zone_id == room.zone_id)
				continue
			// If nearby is a wall, we'll punch through it
			var/score = 1000
			if(state.geometry.wall_lookup[nearby_turf])
				score -= 200  // Penalize wall punching, but still allow
			if(state.geometry.boundary_lookup[nearby_turf])
				score -= 400  // Penalize boundary punching more
			if(istype(target_turf))
				score -= abs(room_turf.x - target_turf.x) + abs(room_turf.y - target_turf.y)
			if(score > best_score)
				best_opening_turf = room_turf
				best_pair_turf = nearby_turf
				best_dir = check_dir
				best_target_zone_id = nearby_zone_id
				best_score = score
	if(!istype(best_opening_turf))
		return null
	// Punch through wall if needed
	if(state.geometry.wall_lookup[best_pair_turf])
		state.geometry.wall_lookup -= best_pair_turf
		state.geometry.internal_wall_turfs -= best_pair_turf
	if(state.geometry.boundary_lookup[best_pair_turf])
		state.geometry.boundary_lookup[best_pair_turf] = FALSE
	return list(
		"opening_turf" = best_opening_turf,
		"pair_turf" = best_pair_turf,
		"opening_dir" = best_dir,
		"target_zone_id" = best_target_zone_id,
	)
...
/datum/world_edit_generator/building_layout/proc/ensure_room_first_required_room_access(datum/world_edit_building_layout_state/state)
	...
	for(var/datum/world_edit_building_room/room as anything in state.geometry.solved_rooms)
		...
		if(!istype(opening_turf))
			...
			if(!istype(opening_turf))
				opening = ensure_room_first_emergency_door(state, room)
				opening_turf = opening ? opening["opening_turf"] : null
				if(!istype(opening_turf))
					if(zone_spec.required)
...
/datum/world_edit_generator/building_layout/proc/room_first_room_touches_zone(datum/world_edit_building_layout_state/state, datum/world_edit_building_room/room, zone_id)
	if(!istype(state) || !istype(room) || !length("[zone_id]"))
		return FALSE
	for(var/turf/room_turf as anything in room.turfs)
		if(!istype(room_turf))
			continue
...
/datum/world_edit_generator/building_layout/proc/building_room_has_internal_door(datum/world_edit_building_layout_state/state, datum/world_edit_building_room/room)
	...
	for(var/turf/door_turf as anything in state.geometry.door_turfs)
		...
		var/datum/world_edit_building_zone_spec/pair_zone_spec = state.semantic_plan?.get_zone_spec(pair_zone_id)
		if(istype(pair_zone_spec) && pair_zone_id != room.zone_id)
			return TRUE
		// Entry zone rooms are accessible via the front door even when the pair turf is outside the footprint
		if(istype(state.semantic_plan) && room.zone_id == state.semantic_plan.entry_zone_id)
			if(door_turf == state.geometry.front_door_turf)
				return TRUE
	return FALSE
...
/datum/world_edit_generator/building_layout/proc/build_building_rooms_from_zone_assignments(datum/world_edit_building_layout_state/state)
	...
	var/use_component_rooms = (round(text2num("[state.config["target_room_count"]]") || 0) > 0)
	for(var/datum/world_edit_building_zone_spec/zone_spec as anything in state.semantic_plan.zone_specs)
		if(!istype(zone_spec))
			continue
		if(!use_component_rooms)
			var/datum/world_edit_building_room/room = new("room_[zone_spec.id]_[length(state.geometry.solved_rooms) + 1]", zone_spec.id, zone_spec.role)
			...
			state.add_solved_room(room)
			continue
		var/list/components = build_building_room_components_for_zone(state, zone_spec.id)
		...
		for(var/list/component_turfs as anything in components)
			...
			for(var/turf/component_turf as anything in component_turfs)
				if(istype(component_turf))
					room.add_turf(component_turf)
			if(!length(room.turfs))
				continue
			room.focus_turf = select_building_room_component_focus(state, zone_spec.id, room.turfs)
			room.tiny = length(room.turfs) <= 1
			state.add_solved_room(room)
	refresh_building_zone_foci(state)
...
/datum/world_edit_generator/building_layout/proc/build_building_room_walkable_lookup_for_zone(datum/world_edit_building_layout_state/state, zone_id, list/planned_wall_lookup = null, list/planned_opening_lookup = null)
	var/list/walkable_lookup = list()
	if(!istype(state) || !length("[zone_id]"))
		return walkable_lookup
	for(var/turf/zone_turf as anything in state.get_zone_turfs(zone_id))
		if(!istype(zone_turf) || state.geometry.boundary_lookup[zone_turf] || state.geometry.wall_lookup[zone_turf] || state.geometry.door_dirs[zone_turf])
			continue
		if(islist(planned_wall_lookup) && planned_wall_lookup[zone_turf])
			continue
		if(islist(planned_opening_lookup) && planned_opening_lookup[zone_turf])
			continue
		walkable_lookup[zone_turf] = TRUE
	return walkable_lookup

/datum/world_edit_generator/building_layout/proc/build_building_room_components_for_zone(datum/world_edit_building_layout_state/state, zone_id, list/planned_wall_lookup = null, list/planned_opening_lookup = null)
	var/list/components = list()
	var/list/walkable_lookup = build_building_room_walkable_lookup_for_zone(state, zone_id, planned_wall_lookup, planned_opening_lookup)
	var/list/visited_lookup = list()
	for(var/turf/start_turf as anything in walkable_lookup)
		...
		while(index <= length(queue))
			...
			for(var/check_dir in GLOB.cardinals)
				...
				if(!walkable_lookup[nearby_turf] || visited_lookup[nearby_turf])
					continue
...
/datum/world_edit_generator/building_layout/proc/count_building_room_components(datum/world_edit_building_layout_state/state, list/planned_wall_lookup = null, list/planned_opening_lookup = null)
	if(!istype(state) || !istype(state.semantic_plan))
		return 0
	...
	for(var/datum/world_edit_building_zone_spec/zone_spec as anything in state.semantic_plan.zone_specs)
		if(!istype(zone_spec))
			continue
		component_count += length(build_building_room_components_for_zone(state, zone_spec.id, planned_wall_lookup, planned_opening_lookup))
	return component_count

/datum/world_edit_generator/building_layout/proc/select_building_room_component_focus(datum/world_edit_building_layout_state/state, zone_id, list/component_turfs)
	if(!istype(state) || !islist(component_turfs) || !length(component_turfs))
		return null
	var/turf/zone_focus = state.get_zone_focus(zone_id)
	if(istype(zone_focus) && (zone_focus in component_turfs))
		return zone_focus
	var/turf/target_turf = zone_focus || state.geometry.semantic_hub_turf || state.geometry.center_turf || state.geometry.front_door_turf
	var/turf/best_turf = null
	var/best_score = -999999999
	for(var/turf/component_turf as anything in component_turfs)
		if(!istype(component_turf))
		...
		var/score = 0
		if(istype(target_turf))
			score -= abs(component_turf.x - target_turf.x) + abs(component_turf.y - target_turf.y)
		if(state.geometry.corridor_lookup[component_turf])
			score += 20
		if(!istype(best_turf) || score > best_score)
			best_turf = component_turf
			best_score = score
	return best_turf || component_turfs[1]

/datum/world_edit_generator/building_layout/proc/build_building_requested_room_dividers(datum/world_edit_building_layout_state/state)
	if(!istype(state) || !istype(state.semantic_plan))
		return
	var/target_room_count = round(text2num("[state.config["target_room_count"]]") || 0)
	if(target_room_count <= 0)
		return
	state.validation.requested_room_count = target_room_count
	var/current_room_count = count_building_room_components(state)
	if(current_room_count >= target_room_count)
		return
	var/max_divider_count = min(target_room_count - current_room_count, 12)
	var/built_divider_count = 0
	var/attempt_count = 0
	while(current_room_count < target_room_count && built_divider_count < max_divider_count && attempt_count < 24)
		attempt_count++
		var/datum/world_edit_building_divider_plan/divider_plan = select_requested_room_count_divider_plan(state, current_room_count, "requested_[attempt_count]")
		if(!istype(divider_plan))
			break
		var/list/planned_wall_lookup = GLOB.world_edit_placement_shapes.world_edit_build_turf_lookup(divider_plan.wall_turfs)
		var/list/planned_opening_lookup = GLOB.world_edit_placement_shapes.world_edit_build_turf_lookup(divider_plan.opening_turfs)
		var/planned_room_count = count_building_room_components(state, planned_wall_lookup, planned_opening_lookup)
		if(planned_room_count <= current_room_count)
			break
		state.add_divider_plan(divider_plan)
		state.validation.room_count_divider_count++
		built_divider_count++
		current_room_count = count_building_room_components(state)

/datum/world_edit_generator/building_layout/proc/select_requested_room_count_divider_plan(datum/world_edit_building_layout_state/state, current_room_count, sequence_id)
	if(!istype(state) || !istype(state.semantic_plan))
		return null
	var/datum/world_edit_building_divider_plan/best_plan = null
	var/best_score = -999999999
	for(var/datum/world_edit_building_zone_spec/zone_spec as anything in state.semantic_plan.zone_specs)
		if(!istype(zone_spec))
			continue
		var/datum/world_edit_building_divider_plan/candidate = build_requested_room_count_divider_plan_for_zone(state, zone_spec, current_room_count, sequence_id)
		if(!istype(candidate))
			continue
		var/score = candidate.score || 0
		if(!istype(best_plan) || score > best_score)
			best_plan = candidate
			best_score = score
	return best_plan

/datum/world_edit_generator/building_layout/proc/build_requested_room_count_divider_plan_for_zone(datum/world_edit_building_layout_state/state, datum/world_edit_building_zone_spec/zone_spec, current_room_count, sequence_id)
	if(!istype(state) || !istype(zone_spec))
		return null
	if(!building_zone_allows_requested_room_count_divider(state, zone_spec))
		return null
	var/list/walkable_lookup = build_building_room_walkable_lookup_for_zone(state, zone_spec.id)
	if(length(walkable_lookup) < max(8, zone_spec.min_area * 2))
		return null
	var/min_x = 999999
	var/max_x = -999999
	var/min_y = 999999
	var/max_y = -999999
	for(var/turf/walkable_turf as anything in walkable_lookup)
		if(!istype(walkable_turf))
			continue
		min_x = min(min_x, walkable_turf.x)
		max_x = max(max_x, walkable_turf.x)
		min_y = min(min_y, walkable_turf.y)
		max_y = max(max_y, walkable_turf.y)
	var/datum/world_edit_building_divider_plan/best_plan = null
	var/best_score = -999999999
	var/list/orientations = list("vertical", "horizontal")
	for(var/orientation as anything in orientations)
		var/start_axis = orientation == "vertical" ? min_x + 2 : min_y + 2
		var/end_axis = orientation == "vertical" ? max_x - 2 : max_y - 2
		for(var/axis_value = start_axis, axis_value <= end_axis, axis_value++)
			var/list/candidate_report = build_requested_room_count_divider_candidate(state, zone_spec, walkable_lookup, orientation, axis_value, min_x, max_x, min_y, max_y, current_room_count, sequence_id)
			if(!islist(candidate_report))
				continue
			var/datum/world_edit_building_divider_plan/candidate_plan = candidate_report["plan"]
			if(!istype(candidate_plan))
				continue
			var/score = round(text2num("[candidate_report["score"]]") || 0)
			if(!istype(best_plan) || score > best_score)
...
/datum/world_edit_generator/building_layout/proc/building_zone_allows_requested_room_count_divider(datum/world_edit_building_layout_state/state, datum/world_edit_building_zone_spec/zone_spec)
	if(!istype(state) || !istype(zone_spec))
		return FALSE
	if(zone_spec.privacy_sensitive)
		return FALSE
	var/role = lowertext("[zone_spec.role]")
	if(role in list("private", "service", "storage", "secure", "nested"))
		return FALSE
	return TRUE

/datum/world_edit_generator/building_layout/proc/build_requested_room_count_divider_candidate(datum/world_edit_building_layout_state/state, datum/world_edit_building_zone_spec/zone_spec, list/walkable_lookup, orientation, axis_value, min_x, max_x, min_y, max_y, current_room_count, sequence_id)
	var/list/line_turfs = list()
	var/list/side_a_turfs = list()
	var/list/side_b_turfs = list()
	var/line_min = 999999
	var/line_max = -999999
	for(var/turf/walkable_turf as anything in walkable_lookup)
		if(!istype(walkable_turf))
			continue
		if(orientation == "vertical")
			if(walkable_turf.x == axis_value)
				line_turfs += walkable_turf
				line_min = min(line_min, walkable_turf.y)
				line_max = max(line_max, walkable_turf.y)
			else if(walkable_turf.x < axis_value)
				side_a_turfs += walkable_turf
			else
				side_b_turfs += walkable_turf
		else
			if(walkable_turf.y == axis_value)
				line_turfs += walkable_turf
				line_min = min(line_min, walkable_turf.x)
				line_max = max(line_max, walkable_turf.x)
			else if(walkable_turf.y < axis_value)
				side_a_turfs += walkable_turf
			else
				side_b_turfs += walkable_turf
	if(length(line_turfs) < 3 || length(side_a_turfs) < 3 || length(side_b_turfs) < 3)
		return null
	if((line_max - line_min + 1) != length(line_turfs))
		return null
	var/list/reserved_line_turfs = list()
	for(var/turf/line_turf as anything in line_turfs)
		if(state.geometry.reserved_lookup[line_turf])
			reserved_line_turfs += line_turf
	if(length(reserved_line_turfs) > 1)
		return null
	var/turf/opening_turf = null
	if(length(reserved_line_turfs) == 1)
		opening_turf = reserved_line_turfs[1]
	else
		opening_turf = select_requested_room_count_opening_turf(state, line_turfs, orientation)
	if(!istype(opening_turf))
		return null
	var/list/wall_turfs = list()
	for(var/turf/line_turf as anything in line_turfs)
		if(!istype(line_turf) || line_turf == opening_turf)
			continue
		if(state.geometry.reserved_lookup[line_turf] || state.geometry.wall_lookup[line_turf] || state.geometry.door_dirs[line_turf])
			return null
		wall_turfs += line_turf
	if(length(wall_turfs) < 2)
		return null
	var/datum/world_edit_building_divider_plan/divider_plan = new("roomcount_[zone_spec.id]_[sequence_id]", zone_spec.id, zone_spec.id)
	for(var/turf/wall_turf as anything in wall_turfs)
		divider_plan.wall_turfs += wall_turf
	divider_plan.opening_turfs += opening_turf
	divider_plan.opening_dirs[opening_turf] = get_requested_room_count_opening_dir(state, opening_turf, orientation)
	if(!divider_plan_keeps_floor_reachable(state, divider_plan))
		return null
	var/list/planned_wall_lookup = GLOB.world_edit_placement_shapes.world_edit_build_turf_lookup(divider_plan.wall_turfs)
	var/list/planned_opening_lookup = GLOB.world_edit_placement_shapes.world_edit_build_turf_lookup(divider_plan.opening_turfs)
	...
	if(planned_room_count <= current_room_count)
		return null
	var/balance_penalty = abs(length(side_a_turfs) - length(side_b_turfs))
	var/score = min(length(side_a_turfs), length(side_b_turfs)) * 100 - balance_penalty * 6 + length(wall_turfs) * 3
	if(state.geometry.reserved_lookup[opening_turf])
		score += 120
	if(zone_spec.id == state.semantic_plan?.primary_zone_id || zone_spec.id == state.semantic_plan?.hub_zone_id)
		score += 2000
	var/role = lowertext("[zone_spec.role]")
	if(role in list("hub", "public", "public_med", "route", "entry", "staging"))
		score += 500
	divider_plan.score = score
	return list("plan" = divider_plan, "score" = score)

/datum/world_edit_generator/building_layout/proc/select_requested_room_count_opening_turf(datum/world_edit_building_layout_state/state, list/line_turfs, orientation)
	var/turf/best_turf = null
	var/best_score = -999999999
	var/turf/target_turf = state.geometry.semantic_hub_turf || state.geometry.front_door_turf || state.geometry.center_turf
	for(var/turf/line_turf as anything in line_turfs)
		if(!istype(line_turf) || state.geometry.wall_lookup[line_turf] || state.geometry.door_dirs[line_turf] || state.geometry.boundary_lookup[line_turf])
			continue
		var/score = 0
		if(istype(target_turf))
			score -= abs(line_turf.x - target_turf.x) + abs(line_turf.y - target_turf.y)
		if(state.geometry.corridor_lookup[line_turf])
			score += 80
		var/turf/side_a = get_step(line_turf, orientation == "vertical" ? WEST : SOUTH)
		var/turf/side_b = get_step(line_turf, orientation == "vertical" ? EAST : NORTH)
		if(!state.geometry.footprint_lookup[side_a] || !state.geometry.footprint_lookup[side_b])
			score -= 1000
		if(!istype(best_turf) || score > best_score)
			best_turf = line_turf
			best_score = score
...
/datum/world_edit_generator/building_layout/proc/get_requested_room_count_opening_dir(datum/world_edit_building_layout_state/state, turf/opening_turf, orientation)
	var/turf/target_turf = state.geometry.semantic_hub_turf || state.geometry.front_door_turf || state.geometry.center_turf
	if(orientation == "vertical")
		if(istype(target_turf) && target_turf.x < opening_turf.x)
			return WEST
		return EAST
	if(istype(target_turf) && target_turf.y < opening_turf.y)
		return SOUTH
	return NORTH
...
/datum/world_edit_generator/building_layout/proc/build_building_walls_and_floors(datum/world_edit_building_layout_state/state)
	...
	var/list/source_turfs = length(state.geometry.footprint) ? state.geometry.footprint : state.geometry.interior
	for(var/turf/footprint_turf as anything in source_turfs)
