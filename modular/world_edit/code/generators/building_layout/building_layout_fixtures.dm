/datum/world_edit_generator/building_layout/proc/place_building_fixtures(datum/world_edit_building_layout_state/state)
	...
	if(detail_budget < 65)
		place_building_room_purpose_fill(state, detail_budget)
		return
...
/datum/world_edit_generator/building_layout/proc/place_building_room_purpose_fill(datum/world_edit_building_layout_state/state, detail_budget)
	if(!istype(state) || state.has_errors() || !istype(state.semantic_plan))
		return
	detail_budget = clamp(round(text2num("[detail_budget]") || 0), 0, 100)
	if(detail_budget <= 0)
		return
	var/remaining_new_fixtures = min(WORLD_EDIT_BUILDING_MAX_CLUSTER_STEPS, max(WORLD_EDIT_BUILDING_MAX_FIXTURE_OBJECTS - state.fixtures.fixture_count, 0))
	if(remaining_new_fixtures <= 0)
		return
	for(var/datum/world_edit_building_room/room as anything in state.geometry.solved_rooms)
		if(!istype(room) || remaining_new_fixtures <= 0)
			break
		var/target_extra = get_building_room_purpose_fill_target(state, room, detail_budget)
		if(target_extra <= 0)
			continue
		var/list/tried_cluster_ids = list()
		var/placed_for_room = 0
		var/attempts = 0
		while(placed_for_room < target_extra && attempts < WORLD_EDIT_BUILDING_MAX_CLUSTER_STEPS && remaining_new_fixtures > 0)
			attempts++
			var/datum/world_edit_building_cluster_spec/cluster_spec = select_building_room_purpose_fill_cluster(state, room, tried_cluster_ids)
			if(!istype(cluster_spec))
				break
			tried_cluster_ids[cluster_spec.id] = TRUE
			var/before_count = state.fixtures.fixture_count
			if(!place_building_cluster_spec(state, cluster_spec, FALSE))
				continue
			var/placed_now = max(state.fixtures.fixture_count - before_count, 0)
			if(placed_now <= 0)
				continue
			placed_for_room += placed_now
			remaining_new_fixtures -= placed_now
			state.validation.room_fill_attempt_count++
			state.validation.room_fill_fixture_count += placed_now

/datum/world_edit_generator/building_layout/proc/get_building_room_purpose_fill_target(datum/world_edit_building_layout_state/state, datum/world_edit_building_room/room, detail_budget)
	if(!istype(state) || !istype(room) || !length(room.turfs))
		return 0
	var/open_floor = 0
	var/existing_fixtures = 0
	for(var/turf/room_turf as anything in room.turfs)
		if(!istype(room_turf) || state.geometry.wall_lookup[room_turf] || state.geometry.door_dirs[room_turf])
			continue
		if(state.geometry.reserved_lookup[room_turf])
			continue
	...
	if(open_floor <= 0)
		return 0
	var/room_area = open_floor + existing_fixtures
	var/desired_total = round((room_area * max(detail_budget, 25)) / 140)
	if(room_area >= 12)
		desired_total = max(desired_total, 1)
	if(room_area >= 28)
		desired_total = max(desired_total, 2)
	desired_total = min(desired_total, max(round(room_area / 3), 1), 6)
	return min(max(desired_total - existing_fixtures, 0), open_floor, 4)

/datum/world_edit_generator/building_layout/proc/select_building_room_purpose_fill_cluster(datum/world_edit_building_layout_state/state, datum/world_edit_building_room/room, list/tried_cluster_ids)
	if(!istype(state) || !istype(room))
		return null
	var/list/best_specs = list()
	...
	for(var/phase_id as anything in list("secondary", "detail"))
		for(var/datum/world_edit_building_cluster_spec/cluster_spec as anything in state.semantic_plan.get_cluster_specs(phase_id))
			if(!building_room_purpose_fill_cluster_is_eligible(state, room, cluster_spec, tried_cluster_ids))
				continue
			var/score = score_building_room_purpose_fill_cluster(state, room, cluster_spec)
			if(score > best_score)
				best_score = score
				best_specs.Cut()
				best_specs += cluster_spec
			else if(score == best_score)
				best_specs += cluster_spec
	if(!length(best_specs))
		return null
	return state.request.fixture_rng.pick_from(best_specs)

/datum/world_edit_generator/building_layout/proc/building_room_purpose_fill_cluster_is_eligible(datum/world_edit_building_layout_state/state, datum/world_edit_building_room/room, datum/world_edit_building_cluster_spec/cluster_spec, list/tried_cluster_ids)
	if(!istype(state) || !istype(room) || !istype(cluster_spec) || cluster_spec.required)
		return FALSE
	if(islist(tried_cluster_ids) && tried_cluster_ids[cluster_spec.id])
		return FALSE
	if(!building_cluster_can_use_procedural_pattern(cluster_spec) && !building_cluster_has_template_chunk(cluster_spec))
		return FALSE
	var/budget = state.get_category_budget(cluster_spec.category)
	if(isnum(budget) && budget > 0 && (state.fixtures.category_counts["[cluster_spec.category]"] || 0) >= budget)
		return FALSE
	return count_building_room_purpose_fill_anchor_overlap(state, room, cluster_spec) > 0

/datum/world_edit_generator/building_layout/proc/score_building_room_purpose_fill_cluster(datum/world_edit_building_layout_state/state, datum/world_edit_building_room/room, datum/world_edit_building_cluster_spec/cluster_spec)
	var/score = cluster_spec.priority
	score += count_building_room_purpose_fill_anchor_overlap(state, room, cluster_spec) * 160
	if(room.zone_id in cluster_spec.anchors)
		score += 500
	...
	if(istype(zone_spec))
		for(var/anchor_tag as anything in zone_spec.anchor_tags)
			if("[anchor_tag]" in cluster_spec.anchors)
				score += 180
		if("[zone_spec.role]" in cluster_spec.anchors)
			score += 120
	score -= get_building_repeat_penalty_score(state, cluster_spec.category)
	return score

/datum/world_edit_generator/building_layout/proc/count_building_room_purpose_fill_anchor_overlap(datum/world_edit_building_layout_state/state, datum/world_edit_building_room/room, datum/world_edit_building_cluster_spec/cluster_spec)
	if(!istype(state) || !istype(room) || !istype(cluster_spec))
		return 0
	var/list/candidates = get_fixture_candidate_turfs_for_anchors(state, cluster_spec.anchors)
...
/datum/world_edit_generator/building_layout/proc/build_building_cluster_planned_slot_turfs(datum/world_edit_building_layout_state/state, datum/world_edit_building_cluster_spec/cluster_spec, list/anchor_ids, target_count)
	...
	while(planned_credit_count < target_count && length(candidates))
		...
		state.reserve_semantic_slot(requirement_id, best_turf)
