/datum/species/ruuhtian
	...
	display_name = "Ruuhtian"
	display_name_plural = "Ruuhtians"
...
/datum/species/ruuhtian/New()
	equip_adjust = list(
		WEAR_R_HAND = list("[NORTH]" = list("x" = 5, "y" = -2), "[EAST]" = list("x" = 5, "y" = -2), "[SOUTH]" = list("x" = -4, "y" = -2), "[WEST]" = list("x" = 0, "y" = -2)),
		WEAR_L_HAND = list("[NORTH]" = list("x" = -4, "y" = -2), "[EAST]" = list("x" = 0, "y" = -2), "[SOUTH]" = list("x" = 5, "y" = -2), "[WEST]" = list("x" = -5, "y" = -2))
	)
	..()
...
/datum/species/ruuhtian/handle_post_spawn(mob/living/carbon/human/ruuhtian)
	...
	for(var/obj/limb/limb in ruuhtian.limbs)
		switch(limb.name)
			...
			if("l_hand","r_hand","r_foot","l_foot","r_leg","r_arm","l_leg","l_arm")
				limb.min_broken_damage = 150
				limb.max_damage = 150
				limb.time_to_knit = 1 MINUTES

	ruuhtian.set_languages(list(LANGUAGE_SANGHEILI, LANGUAGE_RUUHTIAN))
	ruuhtian.halo_apply_species_tts_seed()
	return ..()
