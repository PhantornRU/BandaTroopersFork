...
/datum/component/jump/proc/can_jump(atom/movable/jumper)
	...
	if(isliving(jumper))
		...
		if(living_jumper.buckled || living_jumper.is_mob_incapacitated())
			return FALSE
...
/datum/component/jump/proc/do_jump(atom/movable/jumper)
	...
	jumper.add_filter(JUMP_COMPONENT, 2, drop_shadow_filter(color = "#03020781", size = 0.9))
	var/shadow_filter = jumper.get_filter(JUMP_COMPONENT)

	animate(jumper, pixel_z = jumper.pixel_z + effective_jump_height, layer = max(ABOVE_MOB_LAYER - 0.03, original_layer), time = effective_jump_duration / 2, easing = CIRCULAR_EASING|EASE_OUT, flags = ANIMATION_END_NOW|ANIMATION_PARALLEL)
	...
	if(jump_flags & JUMP_SPIN)
		var/spin_number = max(CEILING(effective_jump_duration * 0.1, 1), 1)
		jumper.animation_spin(effective_jump_duration / spin_number, spin_number, jumper.dir != WEST, anim_flags = ANIMATION_PARALLEL)
...
/datum/component/jump/proc/jump_throw(atom/movable/thrower, target, thrown_thing, list/throw_modifiers)
	...
	if(!istype(throw_item) || throw_item.w_class > SIZE_MEDIUM)
		return
