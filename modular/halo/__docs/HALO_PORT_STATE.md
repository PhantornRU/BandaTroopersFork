...
- Current port wave: `CORE + SQUADS-owned UNSC ship runtime`
...
- Active HALO ship maps use HALO platoon families selected by ship-map `platoon` defaults plus optional `allowed_platoons` admin-choice whitelists, then resolved through the generic ship-profile helpers in `modular/squads`.
- Active HALO runtime no longer ships legacy single-squad ODST compat datums; ship content must use the HALO platoon families owned by `modular/squads`.
...
- HALO squad/role string contracts intentionally remain in the early-include upstream compat surface `code/__DEFINES/bandamarines/halo_jobs.dm`; `colonialmarines.dme` includes that file before `code/__DEFINES/mode.dm` so shared marine-role lists can consume the same macros without duplicating literals while staying DME-order compliant.
- Legacy single-squad ODST compat paths were intentionally removed; active HALO ship maps and code must target `/datum/squad/marine/halo/odst/*` and `/datum/job/marine/.../halo/odst`.
- Ship-map `gamemodes` are no longer used as a HALO ship selection surface; HALO ship-map platoon choice now comes from ship JSON `platoon` defaults plus optional `allowed_platoons` admin choices written into `data/next_ship.json`.
...
- Recheck HALO squad-role and ship-locker ownership surfaces in `modular/squads`: `modular/squads/code/__DEFINES/halo_unsc_crew_jobs.dm`, `modular/squads/code/job/{halo_modular_platoons,ship_platoon_profiles}.dm`, and `modular/squads/code/closets/{halo_marine_personal,halo_marine_personal_squads,halo_job_lockers,halo_unsc_crew_personal}.dm`.
- Recheck ODST/HALO glue surfaces in `code/**`: `code/__DEFINES/{job,mode}.dm`, `code/__DEFINES/bandamarines/halo_jobs.dm`, `colonialmarines.dme`, `code/controllers/subsystem/communications.dm`, `code/game/jobs/job/marine/{squads.dm,squad/*}.dm`, `code/modules/client/preferences.dm`, `code/modules/mob/new_player/preferences_setup.dm`, `code/game/gamemodes/colonialmarines/colonialmarines.dm`, `code/modules/mob/living/carbon/human/human.dm`, and `code/modules/unit_tests/halo_ship_platoons.dm`.
...
- Recheck HALO map activation/config surfaces: `map_config/maps.txt`, `map_config/shipmaps.txt`, `code/datums/map_config.dm`, `code/modules/admin/server_verbs.dm`, and HALO ship JSON configs in `maps/{unsc_stalwart_frigate,unsc_dark_was_the_night,unsc_dark_was_the_night_odst}.json`.
