...
- `PR #46` Mackay/ONI Digsite shuttle IDs and map item contracts are covered by BT-specific shared glue in `code/__DEFINES/bandamarines/halo_map_support.dm` and `code/modules/cm_marines/equipment/maps.dm`; these are intentionally not duplicated in `code/__DEFINES/shuttles.dm`.
