...
- HALO squad/role string contracts intentionally remain in the early-include upstream compat surface `code/__DEFINES/halo_jobs.dm`; `colonialmarines.dme` includes that file before `code/__DEFINES/mode.dm` so shared marine-role lists can consume the same macros without duplicating literals while staying DME-order compliant.
