...
/obj/structure/machinery/door_control/squad_armory/proc/has_captain_authorization(mob/living/user)
	...
	var/mob/living/carbon/human/human_user = user
	return is_captain_or_higher_role(human_user) || is_unsc_crew_role(human_user) || user_has_any_access(human_user, list(ACCESS_MARINE_CO))
...
/obj/structure/machinery/door_control/squad_armory/proc/is_captain_or_higher_role(mob/living/carbon/human/human_user)
	...
	switch(GET_DEFAULT_ROLE(human_user.job))
		if(JOB_CO, JOB_COLONEL, JOB_GENERAL, JOB_ACMC, JOB_CMC, JOB_UNSC_CREW_COM_CPT)
			return TRUE

	return FALSE

/obj/structure/machinery/door_control/squad_armory/proc/is_unsc_crew_role(mob/living/carbon/human/human_user)
	if(!ishuman(human_user))
		return FALSE

	switch(human_user.job)
		if(
			JOB_UNSC_CREW,
			JOB_UNSC_CREW_ENGI,
			JOB_UNSC_CREW_ENGI_CHIEF,
			JOB_UNSC_CREW_FLIGHT,
...
/obj/structure/machinery/door_control/squad_armory/proc/deny_squad_armory_access(mob/living/user, datum/squad/target_squad)
	if(target_squad?.usable)
		...
		to_chat(user, SPAN_DANGER("[target_squad?.name || "This squad"] is unavailable this round. Captain or UNSC crew access is required to open this armory."))
